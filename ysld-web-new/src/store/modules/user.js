...
import { ACCESS_TOKEN, USER_NAME,USER_INFO,UI_CACHE_DB_DICT_DATA,USER_ID,USER_LOGIN_NAME,CACHE_INCLUDED_ROUTES, PERMISSION_LIST_CACHE } from "@/store/mutation-types"
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        ...
        queryPermissionsByUser(params).then(response => {
          ...
          if (menuData && menuData.length > 0) {
            ...
            Vue.ls.set(PERMISSION_LIST_CACHE, menuData, 30 * 60 * 1000)
          } else {
    ...
    Logout({ commit, state }) {
      return new Promise((resolve) => {
        ...
        Vue.ls.remove(PERMISSION_LIST_CACHE)
        logout().then(() => {
          resolve()
        }).catch(() => {
          resolve()
