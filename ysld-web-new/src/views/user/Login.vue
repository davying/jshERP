  ...
  <div class="login-page">
    <div class="login-content">
      <div class="company-header">
        <span class="company-name">{{ systemTitle }}</span>
        <span class="company-version">{{ systemVersion }}</span>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        systemVersion: window.SYS_VERSION || 'v1.0',
        ...
        if (that.customActiveKey === 'tab1') {
          that.form.validateFields([ 'loginName', 'password', 'rememberMe' ], { force: true }, (err, values) => {
            if (!err) {
              ...
              loginParams.passwordStr = values.password
      ...
      departConfirm(res, loginName){
        if(res.code==200){
          ...
          if(res.data.msgTip == 'user can login'){
            ...
            Vue.ls.set('roleName', res.data.roleName, 7 * 24 * 60 * 60 * 1000);
            Vue.ls.set('loginUser', res.data.user, 7 * 24 * 60 * 60 * 1000);
...
<style lang="less" scoped>
  .login-page {
    ...
    padding: 24px;
    background: linear-gradient(160deg, #f4f6fb 0%, #eef3ff 100%);
  ...
  .user-layout-login {
    ...
    /deep/ .ant-input:focus {
      border-color: #4a7bff;
      box-shadow: 0 0 0 2px rgba(74, 123, 255, 0.12);
    }
    ...
    button.login-button {
      ...
      border: none;
      border-radius: 6px;
      background: linear-gradient(90deg, #3b74ff 0%, #4f8dff 100%);
      box-shadow: 0 6px 14px rgba(63, 120, 255, 0.25);
    ...
    button.login-button:focus {
      ...
      box-shadow: 0 8px 18px rgba(63, 120, 255, 0.35);
    }
