...
export const BillModalMixin = {
  ...
  methods: {
        ...
        if(res) {
          ...
          for(let item of that.materialTable.columns){
              ...
              for(let i=0; i<arr.length; i++) {
                let depotInfo = {};
                depotInfo.value = arr[i].commodityNo + '' //注意-此处value必须为字符串格式
                depotInfo.text = arr[i].name
                depotInfo.title = arr[i].name + "(" + arr[i].commodityNo + ")"
    ...
    initLogisticsOut(){
      let that = this;
      postAction('/depotHead/getDepotHeadIds', {status:'4,9',type:'发货'}).then((res)=>{
        if(res) {
          let arr = res.data
          for(let item of that.materialTable.columns) {
            if(item.key == 'depotIds') {
              item.options = []
              for (let i = 0; i < arr.length; i++) {
                let depotInfo = {};
                depotInfo.value = arr[i].number + '',
                    depotInfo.text = arr[i].number + "（" + arr[i].salesMan + "）",
                    depotInfo.title = arr[i].number + "（" + arr[i].salesMan + "）",
                    depotInfo.color = arr[i].color
                item.options.push(depotInfo)
              }
            }
            if(item.key == 'type') {
              item.options = []
              let depotInfo = {};
              depotInfo.value = '全部发货',
                  depotInfo.text = '全部发货',
                  depotInfo.title = '全部发货'
              item.options.push(depotInfo)
              depotInfo = {};
              depotInfo.value = '部分发货',
                  depotInfo.text = '部分发货',
                  depotInfo.title = '部分发货'
              item.options.push(depotInfo)
            }
          }
        }
    ...
    initSaleOutNew() {
      let that = this;
      postAction('/depotHead/getDepotHeadIds', {status:'4,9',type:'出库'}).then((res)=>{
        if(res) {
          let arr = res.data
          for(let item of that.materialTable.columns) {
            if(item.key == 'depotIds') {
            ...
            if(item.key == 'type') {
              item.options = []
              let depotInfo = {};
              depotInfo.value = '全部出库',
              depotInfo.text = '全部出库',
              depotInfo.title = '全部出库'
              item.options.push(depotInfo)
              depotInfo = {};
              depotInfo.value = '部分出库',
              depotInfo.text = '部分出库',
              depotInfo.title = '部分出库'
              item.options.push(depotInfo)
            }
          }
        }
    ...
    onValueChange(event) {
      ...
      let commodityNo,color,size1,size2,size3,size4,size5,size6,size7,size8,size9,size10,size11,numbers,inTime,price,remark,param,snList,batchNumber,operNumber,unitPrice,allPrice,taxRate,taxMoney,taxLastMoney
      switch(column.key) {
        case "depotIds":
          commodityNo = row.depotIds
          if(row.depotIds){
            if(column.title.indexOf('发货')>-1){
              that.getDepotById('发货',row, target)
            }else{
              that.getDepotById('出库',row, target)
            }
          }
          break;
        case "commodityNo":
    ...
    // 根据订单编号获取详情 TODO 插入行
    getDepotById(type,row, target){
      getAction('/depotHead/checkNumberOutInfo',{ type:type,depotId: row.depotIds}).then((res) => {
        if (res && res.code ==1) {
          target.setValues([{rowKey: row.id, values:
                {commodityNo: res.data.commodityNo,
                  color: res.data.color,
                  salesMan: res.data.salesMan,
                  freightPayer: res.data.freightPayer,
                  size1: res.data.size1,
                  size2: res.data.size2,
                  size3: res.data.size3,
                  size4: res.data.size4,
                  size5: res.data.size5,
                  size6: res.data.size6,
                  size7: res.data.size7,
                  size8: res.data.size8,
                  size9: res.data.size9,
                  size10: res.data.size10,
                  size11: res.data.size11,
                  actualQuantity: res.data.numbers,
          }}])
        }
      })
    },
