  ...
  export default {
    ...
    data () {
      return {
        ...
        saleOrderColumns: [
          { title: '货品编码', dataIndex: 'commodityNo'},
    ...
    methods: {
      initSetting(record, type, ds) {
        ...
        if(record.status === '3') {
        ...
        } else if(record.purchaseStatus === '3'|| record.status == '0') {
          ...
          for(let i=0; i<this.defColumns.length; i++){
            ...
            if(needRemoveKeywords.indexOf(this.defColumns[i].dataIndex)===-1) {
              ...
              info.title = this.defColumns[i].title
              info.dataIndex = this.defColumns[i].dataIndex
              // info.width = this.defColumns[i].width||50
              console.log(info)
              currentCol.push(info)
            }
            // //添加有数据的列
      ...
      show(record, type, prefixNo) {
        ...
        findBillDetailByNumber({ number: record.number }).then((res) => {
          ...
          if (res && res.code === 200) {
            ...
            this.model.operTime = moment(item.operTime).format("YYYY-MM-DD HH:mm:ss");
