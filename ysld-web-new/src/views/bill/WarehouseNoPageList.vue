    ...
    <a-col :md="24">
      <a-card :style="cardStyle" :bordered="false">
        ...
        <div class="table-operator" style="margin-top: 5px">
          ...
          <a-button type="primary" @click="exportWarehouse()">导出</a-button>
        </div>
        <!-- table区域-begin -->
        <div>
...
<!--          :pagination="ipagination"  注释这个，pagination变成false 多合计-->
          ...
          >
            <span slot="action" slot-scope="text, record">
              <a v-if="text!='total' && btnEnableList.indexOf(1) > -1" @click="handleEdit(record)">编辑</a>
              <a-divider v-if="btnEnableList.indexOf(1) > -1 && text!='total'" type="vertical" />
              <a-popconfirm
                v-if="btnEnableList.indexOf(1) > -1 && text!='total'"
                title="确定删除吗?"
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        exportWarehouse:"/warehouseManagement/exportWarehouse",
      },
    };
  },
  ...
  methods: {
    ...
    exportWarehouse() {
      ...
      downFile(this.url.exportWarehouse, param).then(res => {
        const url = window.URL.createObjectURL(new Blob([res],{type: 'application/vnd.ms-excel'}));
        const link = document.createElement('a');
        link.href = url;
        link.style.display = 'none'
        link.setAttribute('download',  '入库记录_'+getNowFormatStr()+'.xlsx')
