...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: "/sampleManagement/getSampleInOutFinalList",
        delete: "/sampleManagement/delete",
        exportXlsUrl: "/sampleManagement/exportSampleInOutList",
      },
    };
  },
        ...
        this.$confirm({
          ...
          onOk: function () {
            ...
            downFile(that.url.exportXlsUrl, param).then(res => {
              ...
              // link.download = '销售订单-' + formatDate+ '.xls';
              link.style.display = 'none'
              link.setAttribute('download',  '样衣库存管理_'+getNowFormatStr()+'.xls')
              document.body.appendChild(link)
              link.click()
              document.body.removeChild(link); //下载完成移除元素
              window.URL.revokeObjectURL(url); //释放掉blob对象
