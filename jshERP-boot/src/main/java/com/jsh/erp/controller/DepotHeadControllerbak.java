...
//@Api(tags = {"单据管理"})
//public class DepotHeadControllerbak {
//
//    @Resource
...
//        depots.forEach(e -> {
//                    e = depotHeadNewService.getById(e.getId());
////                    // 订单物流费
////                    QueryWrapper<WarehouseManagement> queryWarehouseWrapper = new QueryWrapper<>();
////                    queryWarehouseWrapper.select("IFNULL(sum(logistics_freight),0) as logisticsFreight");
...
////                    queryWarehouseWrapper.like("type", "发货");
////                    Map<String, Object> orderExpressFee = warehouseManagementService.getMap(queryWarehouseWrapper);
////                    if (orderExpressFee == null) {
////                        e.setLogisticsFreight(new BigDecimal(0));
////                    } else {
////                        BigDecimal logisticsFreight = new BigDecimal(StrUtil.toString(orderExpressFee.get("logisticsFreight")));
////                        e.setLogisticsFreight(logisticsFreight.setScale(4, BigDecimal.ROUND_UP));
////                    }
////            BigDecimal rate = null;
////            if (StrUtil.equals("增票11个点", e.getBillingRate())) {
////                rate = e.getTotalFinancialPrice().multiply(new BigDecimal("0.11"));
////            } else if (StrUtil.equals("普票", e.getBillingRate())) {
...
////            "状态，0未审核、1已审核、2完成采购|销售、3部分采购|销售、4仓库已提交、9业务员已提交"
...
//                    }
////                    depotHeadNewService.updateById(e);
//                }
//        );
//
//
...
//                objs[56] = s.getFinancialOtherExpenditureRemarks();
//                objs[57] = s.getDoesFinancialAffairsIncludeTax();
//                objs[58] = s.getFinancialInvoicePayable();
//                objs[59] = s.getLogoMark();
//                objs[60] = s.getBillingRate();
//                objs[61] = StrUtil.toString(s.getFinancialTaxAmount().setScale(4, BigDecimal.ROUND_UP));
//                objs[62] = s.getFinancialCostUnitPrice() == null ? "" : StrUtil.toString(s.getFinancialCostUnitPrice().setScale(4, BigDecimal.ROUND_UP));
//                objs[63] = StrUtil.toString(s.getFinancialCostAmount().setScale(4, BigDecimal.ROUND_UP));
//                objs[64] = s.getLogisticsFreight() == null ? "0" : StrUtil.toString(s.getLogisticsFreight().setScale(4, BigDecimal.ROUND_UP));
//                objs[65] = StrUtil.toString(s.getOrderCommission().setScale(4, BigDecimal.ROUND_UP));
//                objs[66] = StrUtil.isBlank(s.getEndStaff()) ? "否" : s.getEndStaff();
//                objects.add(objs);
//            }
//        }
