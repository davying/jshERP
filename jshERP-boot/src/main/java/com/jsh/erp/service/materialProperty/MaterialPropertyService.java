...
//            }
    ...
    public int insertMaterialProperty(JSONObject obj, HttpServletRequest request) throws Exception {
        MaterialProperty materialProperty = JSONUtil.toBean(obj.toJSONString(), MaterialProperty.class);
        int result = 0;
        try {
            Long isSameName = materialPropertyMapperEx.countsByMaterialProperty2(materialProperty.getNativeName());
            if (isSameName.intValue() > 0) {
                result = 1;
                throw new RuntimeException("商品属性：" + materialProperty.getNativeName() + "已重复！");
            } else {
                //            if (BusinessConstants.DEFAULT_MANAGER.equals(userService.getCurrentUser().getLoginName())) {
